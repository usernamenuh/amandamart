...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
            ...
            <form method="GET" action="{{ route('laporan.pareto') }}" class="space-y-4" id="filterForm">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center space-x-4 text-sm text-gray-500">
                        <div class="flex items-center">
                            ...
                            <span id="itemCount">{{ count($initialItems) }}</span> / <span id="totalCount">{{ $totalItems }}</span> item
                        </div>
                        <div class="flex items-center">
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200" id="tableBody">
                        {{-- Render hanya initial items, sisanya akan di-load via AJAX --}}
                        @forelse($initialItems as $index => $item)
            ...
            <div id="loadingIndicator" class="hidden px-6 py-4 text-center border-t border-gray-200">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let loadedItems = {{ count($initialItems) }};
    ...
    const observer = new IntersectionObserver((entries) => {
        ...
        rootMargin: '100px' // Mulai load 100px sebelum sentinel terlihat
    });
    ...
    function loadMoreItems() {
        ...
        fetch(`{{ route('laporan.pareto.items') }}?${params}`)
            .then(response => response.json())
            .then(data => {
                if (data.success && data.data.length > 0) {
                    // Render items baru
                    data.data.forEach((item, index) => {
                        const row = createTableRow(item, loadedItems + index + 1);
                        tableBody.appendChild(row);
                    });

                    loadedItems += data.data.length;
                    document.getElementById('itemCount').textContent = loadedItems;
    ...
    function createTableRow(item, rank) {
        ...
        const stockIcon = getStockIcon(item.total_qty);

        row.innerHTML = `
            <td class="px-4 py-3 text-sm text-gray-900 font-bold">
                <div class="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
                    <span class="text-xs font-bold text-blue-600">${rank}</span>
                </div>
            </td>
            <td class="px-4 py-3">
                <div class="flex items-center">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="text-sm font-medium text-gray-900">${escapeHtml(item.nama_barang)}</div>
                        ${item.no_barang ? `<div class="text-xs text-gray-500">No: ${escapeHtml(item.no_barang)}</div>` : ''}
                    </div>
                </div>
            </td>
            <td class="px-4 py-3 text-sm text-center">
                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                    <i class="fas fa-calendar-alt mr-1"></i>
                    ${escapeHtml(item.periode_name)}
                </span>
            </td>
            <td class="px-4 py-3 text-sm text-gray-600">
                <span class="truncate">${escapeHtml(item.vendor || '-')}</span>
            </td>
            <td class="px-4 py-3 text-right text-sm font-semibold text-gray-900">
                <div class="flex items-center justify-end">
                    ${stockIcon}
            ...
            <td class="px-4 py-3 text-right text-sm text-gray-900">
                <span class="font-medium">Rp ${formatCurrency(item.harga_satuan)}</span>
            </td>
            <td class="px-4 py-3 text-right text-sm font-bold text-blue-600">
                Rp ${formatCurrency(item.total_nilai)}
            </td>
            <td class="px-4 py-3 text-center">
                <div class="flex flex-col items-center">
                    <div class="w-12 bg-gray-200 rounded-full h-1.5 mb-1">
                        <div class="bg-blue-600 h-1.5 rounded-full transition-all duration-300" style="width: ${Math.min(item.persentase, 100)}%"></div>
                    </div>
                    <span class="text-xs font-medium text-gray-900">${item.persentase}%</span>
                </div>
            </td>
    ...
    function getCategoryBadge(kategori) {
        const badges = {
            'A': '<span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-red-100 text-red-800 border border-red-200"><i class="fas fa-star mr-1"></i>A</span>',
            'B': '<span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-yellow-100 text-yellow-800 border border-yellow-200"><i class="fas fa-circle mr-1"></i>B</span>',
            'C': '<span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-green-100 text-green-800 border border-green-200"><i class="fas fa-dot-circle mr-1"></i>C</span>'
        };
        return badges[kategori] || '';
    }

    function getStockIcon(qty) {
        if (qty < 10) {
            return '<i class="fas fa-exclamation-triangle text-red-500 mr-1 text-xs"></i>';
        } else if (qty < 50) {
            return '<i class="fas fa-exclamation-circle text-yellow-500 mr-1 text-xs"></i>';
        } else {
            return '<i class="fas fa-check-circle text-green-500 mr-1 text-xs"></i>';
        }
    }
    ...
    console.log('ABC Analysis loaded with infinite scroll - Initial {{ count($initialItems) }} items, Total {{ $totalItems }} items');
});
</script>
@endpush
@endsection
